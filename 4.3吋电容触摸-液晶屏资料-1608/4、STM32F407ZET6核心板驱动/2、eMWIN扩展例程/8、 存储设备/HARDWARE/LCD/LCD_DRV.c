...
{ 	
	...
	//FSMC读时序控制寄存器
	FSMC_ReadWriteTimingStructure.FSMC_AddressSetupTime = 0x08; //地址建立时间 
	FSMC_ReadWriteTimingStructure.FSMC_AddressHoldTime = 0x00;  //地址保持时间模式A未用到 
	FSMC_ReadWriteTimingStructure.FSMC_DataSetupTime = 0x0F;    //数据保存时间为16个HCLK 16(1/168M)ns
	...
	FSMC_NORSRAMInitStructure.FSMC_MemoryType = FSMC_MemoryType_NOR;   //SRAM
