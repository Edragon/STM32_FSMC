...
{ 	
	...
	//FSMC读时序控制寄存器
	FSMC_ReadWriteTimingStructure.FSMC_AddressSetupTime = 0x0A; //地址建立时间 24个HCLK =24(1/168M)ns
	FSMC_ReadWriteTimingStructure.FSMC_AddressHoldTime = 0x00;  //地址保持时间模式A未用到 
	FSMC_ReadWriteTimingStructure.FSMC_DataSetupTime = 0x0F;    //数据保存时间为24个HCLK 24(1/168M)ns
